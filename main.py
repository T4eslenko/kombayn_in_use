                 ...
                 if user_input.lower() == 'e':
                     ...
                     try:
                         ...
                         if 0 <= i < len(sessions):
                             ...
                             chats = client.get_dialogs()
                             for chat in chats:
                                 # Инициализируем переменную для подсчета сообщений в каждом чате
                                 count_messages = 0
                                 if isinstance(chat.entity, Channel) or isinstance(chat.entity, Chat):
                                     # Получаем количество сообщений для каждого чата
                                     count_messages = client.get_messages(chat.entity, limit=0).total
                                     # Rest of your existing code...
                                     # Printing chat info along with message count
                                     all_info.append(f"{i} - {chat.title} \033[93m[{count_messages} сообщений]\033[0m")
